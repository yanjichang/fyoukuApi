...
func BarrageList(episodesId int, startTime int, endTime int) (int64, []BarrageData, error){
	...
	num, err := o.Raw("SELECT id,content,`current_time` FROM barrage WHERE status=1 AND " +
		"episodes_id=? AND `current_time`>=? AND `current_time`<? ORDER BY `current_time` ASC",
		episodesId, startTime, endTime).QueryRows(&barrages)
